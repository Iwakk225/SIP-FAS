...
const fetchLaporanData = async () => {
  ...
  try {
    ...
    const validated = laporanArray.filter(l => 
      l.status === 'Tervalidasi' || l.status === 'tervalidasi'
    ).length;
    const inProgress = laporanArray.filter(l => 
      l.status === 'Dalam Proses' || l.status === 'dalam_proses' || l.status === 'dalam proses'
    ).length;
    const completed = laporanArray.filter(l => 
      l.status === 'Selesai' || l.status === 'selesai'
    ).length;
    ...
    laporanArray.forEach(laporan => {
      ...
      else if (lokasiLower.includes('pusat') || lokasiLower.includes('tengah')) wilayahCount['Surabaya Pusat']++;
      else {
  ...
  const DetailLaporanModal = () => {
    ...
    const getStatusColor = (status) => {
      ...
      if (statusLower === 'dalam_proses' || statusLower === 'dalam proses' || statusLower === 'in_progress') return 'bg-blue-100 text-blue-800';
    ...
    const getStatusText = (status) => {
      ...
      if (statusLower === 'dalam_proses' || statusLower === 'dalam proses' || statusLower === 'in_progress') return 'Dalam Proses';
              ...
              {selectedLaporan.kategori && (
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{selectedLaporan.kategori}</p>
                  </div>
                </div>
              )}
              ...
              {selectedLaporan.pelapor_email && (
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{selectedLaporan.pelapor_email}</p>
                  </div>
                </div>
              )}
            </div>
                ...
                {recentActivities.map((activity, index) => (
                  <div key={index} className="flex items-start space-x-3">
                    <div className={`w-2 h-2 rounded-full mt-2 ${
                      ...
                      activity.type.includes('validasi') ? 'bg-blue-500' :
                      'bg-yellow-500'
                    }`}></div>
                    <div>
