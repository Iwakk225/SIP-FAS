...
const StatusPage = () => {
  ...
  const fetchLaporanUser = async () => {
    ...
    try {
      ...
      const response = await axios.get('http://localhost:8000/api/laporan-user', {
        headers: {
      ...
      try {
        ...
        if (storedLaporan) {
          ...
          const userLaporan = laporanArray.filter(l => 
            l.pelapor_email === user?.email || l.pelapor_nama === user?.name
          );
          ...
          <div className="space-y-4">
            {filteredData.length > 0 ? (
              filteredData.map((laporan) => (
                <div key={laporan.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="p-6 space-y-6">
                    ...
                    return (
                      <div key={step.id} className="flex items-start space-x-4">
                        <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center ${
                          ...
                          isCurrent ? step.bgColor : 'bg-gray-200'
                        }`}>
                        ...
                        <div className="flex-1">
                          ...
                          {isCurrent && step.id === 5 && (
                            <div className="mt-3 p-3 bg-red-50 rounded-lg">
                              <p className="text-sm text-red-800 font-medium">Alasan penolakan:</p>
                              <p className="text-sm text-red-700 mt-1">
                                {selectedLaporan.alasan_penolakan || "Fasilitas baru saja diperbaiki / Data tidak valid"}
                              </p>
                            </div>
                          )}
                        </div>
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    <p className="font-medium text-gray-500">Judul Laporan</p>
                    <p className="text-gray-900">{selectedLaporan.judul}</p>
                  </div>
                  <div>
                    <p className="font-medium text-gray-500">Lokasi</p>
                    <p className="text-gray-900">{selectedLaporan.lokasi}</p>
                  </div>
                  <div>
                    <p className="font-medium text-gray-500">Tanggal Lapor</p>
                    <p className="text-gray-900">{formatDate(selectedLaporan.created_at || selectedLaporan.tanggal)}</p>
                  </div>
                  <div className="md:col-span-2">
                    <p className="font-medium text-gray-500">Deskripsi</p>
                    <p className="text-gray-900 mt-1">{selectedLaporan.deskripsi}</p>
                  </div>
                </div>
              </div>
            </div>
